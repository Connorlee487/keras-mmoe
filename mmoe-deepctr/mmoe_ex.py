...
if __name__ == "__main__":
    ...
    data = pd.read_csv('./mmoe-deepctr/byterec_sample.txt', sep='\t',
    ...
    model = MMOE_MED(dnn_feature_columns, task_types=['binary', 'binary'],
    ...
    model = MMOE_n(dnn_feature_columns, task_types=['binary', 'binary'],
                 l2_reg_embedding=1e-5, task_names=target, device=device, noisy_gate=True, noise_temperature=1.0)
    model.compile("adagrad", loss=["binary_crossentropy", "binary_crossentropy"],
                  metrics=['binary_crossentropy'], )
